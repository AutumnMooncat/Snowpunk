...
public class CrankedMod extends AbstractCardModifier {
    public static final String ID = makeID(CrankedMod.class.getSimpleName());
    public static String[] TEXT = CardCrawlGame.languagePack.getCardStrings(ChestnutsRoasting.ID).EXTENDED_DESCRIPTION;

    public int amount = 0;

    public CrankedMod(int num) {
        this.priority = 2;
        amount = num;
    }
/*
    ...
    public boolean shouldApply(AbstractCard card) {
        if (CardModifierManager.hasModifier(card, ID)) {
            CrankedMod heatMod = (CrankedMod) CardModifierManager.getModifiers(card, ID).get(0);
